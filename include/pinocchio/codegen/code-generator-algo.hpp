...
struct CodeGenRNEA : public CodeGenBase<_Scalar>
{
    ...
    void evalFunction(const Eigen::MatrixBase<ConfigVectorType>& q, const Eigen::MatrixBase<TangentVector1>& v,
    ...
    template <typename ConfigVectorType, typename TangentVector1, typename TangentVector2>
    void evalJacobian(const Eigen::MatrixBase<ConfigVectorType>& q, const Eigen::MatrixBase<TangentVector1>& v,
        const Eigen::MatrixBase<TangentVector2>& a)
    {
        Eigen::DenseIndex it = 0;
        x.segment(it, ad_model.nq) = q;
...
struct CodeGenRNEADerivatives : public CodeGenBase<_Scalar>
{
    ...
    CodeGenRNEADerivatives(const Model& model, const std::string& function_name = "partial_rnea",
    ...
    : Base(model, model.nq + 2 * model.nv, 3 * model.nv * model.nv, function_name, library_name)
    {
        ...
        dtau2_dq = MatrixXs::Zero(model.nv * model.nv, model.nv);
        dtau2_dqd = MatrixXs::Zero(model.nv * model.nv, model.nv);
        dtau2_MSO = MatrixXs::Zero(model.nv * model.nv, model.nv);
        dtau2_dqa = MatrixXs::Zero(model.nv * model.nv, model.nv);

        Base::build_jacobian = true;
    }
    ...
    void buildMap()
    {
        ...
        pinocchio::computeRNEADerivativesFaster(
        ...
        const Eigen::MatrixBase<TangentVector2>& a)
    {
        ...
        it_x += ad_model.nq;
        x.segment(it_x, ad_model.nq) = v;
        it_x += ad_model.nv;
        x.segment(it_x, ad_model.nq) = a;
        it_x += ad_model.nv;
        ...
        const Eigen::MatrixBase<TangentVector2>& a)
    {
        ...
        evalJacobian(x);
        Eigen::DenseIndex it_y = 0;
        dtau2_dq = (Base::jac).block(it_y, it_y, ad_model.nv * ad_model.nv, ad_model.nv);
        dtau2_dqa = (Base::jac).block(it_y, 2 * ad_model.nv, ad_model.nv * ad_model.nv, ad_model.nv);
        it_y += ad_model.nv * ad_model.nv;
        dtau2_dqd = (Base::jac).block(it_y, ad_model.nv, ad_model.nv * ad_model.nv, ad_model.nv);
        dtau2_MSO = (Base::jac).block(it_y, 0, ad_model.nv * ad_model.nv, ad_model.nv);
    }

    // --- Getter functions here---------------//
    // These are very expensive here-----------//
    //-- DONT USE------------------------------//

    MatrixXs getdtaudq() { return dtau_dq; }
    MatrixXs getdtaudv() { return dtau_dv; }
    MatrixXs getdtauda() { return dtau_da; }
    MatrixXs getdtaudq2() { return dtau2_dq; }
    MatrixXs getdtaudv2() { return dtau2_dqd; }
    MatrixXs getdtaudqv2() { return dtau2_MSO; }
    MatrixXs getdtaudqa2() { return dtau2_dqa; }
    VectorXs getFun() { return partial_derivatives; }
...
public:
    MatrixXs dtau2_dq, dtau2_dqd, dtau2_MSO, dtau2_dqa;
};
...
struct CodeGenABADerivatives : public CodeGenBase<_Scalar>
{
    ...
    CodeGenABADerivatives(const Model& model, const std::string& function_name = "partial_aba",
        ...
        const Eigen::MatrixBase<TangentVector2>& tau)
    {
        ...
        x.segment(it_x, ad_model.nq) = v;
        it_x += ad_model.nv;
        x.segment(it_x, ad_model.nq) = tau;
        it_x += ad_model.nv;
        ...
        const Eigen::MatrixBase<TangentVector2>& tau)
    {
        ...
        Eigen::DenseIndex it_y = 0;
        dqdd2_dq = (Base::jac).block(it_y, it_y, ad_model.nv * ad_model.nv, ad_model.nv);
        dqdd2_dqtau = (Base::jac).block(it_y, 2 * ad_model.nv, ad_model.nv * ad_model.nv, ad_model.nv);
        it_y += ad_model.nv * ad_model.nv;
        dqdd2_dqd = (Base::jac).block(it_y, ad_model.nv, ad_model.nv * ad_model.nv, ad_model.nv);
        dqdd2_dqv = (Base::jac).block(it_y, 0, ad_model.nv * ad_model.nv, ad_model.nv);
    }
...
public:
    MatrixXs dqdd2_dq, dqdd2_dqd, dqdd2_dqv, dqdd2_dqtau;
};
...
struct CodeGenIntegrate : public CodeGenBase<_Scalar>
{
    ...
    CodeGenIntegrate(const Model& model, const std::string& function_name = "integrate",
    ...
    void evalFunction(const Eigen::MatrixBase<ConfigVectorType1>& q, const Eigen::MatrixBase<TangentVector>& v,
...
struct CodeGenDifference : public CodeGenBase<_Scalar>
{
    ...
    CodeGenDifference(const Model& model, const std::string& function_name = "difference",
    ...
    void evalFunction(const Eigen::MatrixBase<ConfigVectorType1>& q0, const Eigen::MatrixBase<ConfigVectorType2>& q1,
...
struct CodeGenDDifference : public CodeGenBase<_Scalar>
{
    ...
    CodeGenDDifference(const Model& model, const std::string& function_name = "dDifference",
    ...
    void evalFunction(const Eigen::MatrixBase<ConfigVectorType1>& q0, const Eigen::MatrixBase<ConfigVectorType2>& q1,
        const Eigen::MatrixBase<JacobianMatrix>& J, const ArgumentPosition arg)
    {
        // fill x
        Eigen::DenseIndex it = 0;
        x.segment(it, ad_model.nq) = q0;
