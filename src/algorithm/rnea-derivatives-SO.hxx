...
struct ComputeRNEADerivativesSOForwardStep : public fusion::JointUnaryVisitorBase<ComputeRNEADerivativesSOForwardStep<Scalar, Options,
                                          JointCollectionTpl, ConfigVectorType, TangentVectorType1, TangentVectorType2>>
{
        ...
        const Eigen::MatrixBase<TangentVectorType2>& a)
    {
        ...
        ColsBlock J_cols = jmodel.jointCols(data.J); // data.J has all the phi (in ground frame) stacked in columns
        ColsBlock psid_cols = jmodel.jointCols(data.psid);   // psid_cols is the psi_dot in ground frame
        ColsBlock psidd_cols = jmodel.jointCols(data.psidd); // psidd_cols is the psi_dotdot in ground frame
        ColsBlock dJ_cols = jmodel.jointCols(data.dJ);       // This here is phi_dot in ground frame

        J_cols.noalias() = data.oMi[i].act(jdata.S()); // J_cols is just the phi in ground frame for a joint
        ...
        motionSet::motionAction<ADDTO>(ov, psid_cols, psidd_cols); // This ov here is v(p(i)) , psi_dotdot calcs
        ov += vJ;
        oa += (ov ^ vJ) + data.oMi[i].act(jdata.S() * jmodel.jointVelocitySelector(a) + jdata.c());
        motionSet::motionAction(ov, J_cols, dJ_cols); // This here is phi_dot, here ov used is v(p(i)) + vJ
...
template <typename Scalar, int Options, template <typename, int> class JointCollectionTpl, typename tensortype1,
    typename tensortype2, typename tensortype3, typename tensortype4>
struct ComputeRNEADerivativesSOBackwardStep
: public fusion::JointUnaryVisitorBase<ComputeRNEADerivativesSOBackwardStep<Scalar, Options, JointCollectionTpl, tensortype1,
      tensortype2, tensortype3, tensortype4>>
{
    typedef ModelTpl<Scalar, Options, JointCollectionTpl> Model;
    typedef DataTpl<Scalar, Options, JointCollectionTpl> Data;

    typedef boost::fusion::vector<const Model&, Data&, const tensortype1&, const tensortype2&, const tensortype3&,
        const tensortype4&>
        ArgsType;

    template <typename JointModel>
    static void algo(const JointModelBase<JointModel>& jmodel, const Model& model, Data& data,
        const tensortype1& d2tau_dqdq, const tensortype2& d2tau_dvdv, const tensortype3& dtau_dqdv,
    ...
    tensortype1 &d2tau_dqdq_ = const_cast<tensortype1 &>(d2tau_dqdq);
    tensortype2 &d2tau_dvdv_ = const_cast<tensortype2 &>(d2tau_dvdv);
...
<<<<<<< HEAD
    ...
    template <typename ForceDerived, typename M6>
    static void ForceCrossMatrix(const ForceDense<ForceDerived>& f, const Eigen::MatrixBase<M6>& mout)
    {
        M6& mout_ = PINOCCHIO_EIGEN_CONST_CAST(M6, mout);
...
template <typename Scalar, int Options, template <typename, int> class JointCollectionTpl, typename ConfigVectorType,
    typename TangentVectorType1, typename TangentVectorType2, typename tensortype1, typename tensortype2,
    typename tensortype3, typename tensortype4>
inline void computeRNEADerivativesSO(const ModelTpl<Scalar, Options, JointCollectionTpl>& model,
    ...
    const tensortype4& dtau_dadq)
{
    ...
    typedef ComputeRNEADerivativesSOForwardStep<Scalar, Options, JointCollectionTpl, ConfigVectorType, TangentVectorType1,
        TangentVectorType2>
        Pass1;
    ...
    typedef ComputeRNEADerivativesSOBackwardStep<Scalar, Options, JointCollectionTpl, tensortype1, tensortype2, tensortype3,
        tensortype4>
        Pass2;
    for (JointIndex i = (JointIndex)(model.njoints - 1); i > 0; --i) {
        Pass2::run(model.joints[i], typename Pass2::ArgsType(model, data, const_cast<tensortype1&>(d2tau_dqdq),
