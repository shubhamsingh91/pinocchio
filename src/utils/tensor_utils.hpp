...
double get_tens_diff_norm(Eigen::Tensor<double,3> &ten1,Eigen::Tensor<double,3> &ten2 , int n)
{
  ...
  for (int i=0;i<n;i++)
  {
     for (int j=0;j<n;j++)
    {
      for (int k=0;k<n;k++)
      {
        tmp1= tmp1+abs(ten1(k,j,i)-ten2(k,j,i));
      } 
    }
  }
  return tmp1;
...
// Templated
template <typename T>
void hess_assign_fd2_v3( Eigen::Tensor<double,3> &hess, const Eigen::MatrixBase<T>  &mat, int i1,int i2,int j1,int j2, int k)
{
...
// Assigning code here- for assigning a row or a column of the tensor
template <typename T>
void hess_assign(Eigen::Tensor<double,3> &hess, const Eigen::MatrixBase<T> &vec, const int p, const int q, const int r, const int index, const int stride)
{
  if (index==1)
  { 
...
template<typename T>
void hess_get(const Eigen::Tensor<double,3> &hess,Eigen::MatrixBase<T> &vec,int p, const int q, const int r, const int index, const int stride )
{
    if (index==1)
    { 
        for(int ii=0;ii<stride;ii++)
...
template <typename T>
void tens_assign(Eigen::Tensor<double,3> &tens, const Eigen::MatrixBase<T> &mat, const int index, const int x0, const int y0, const int z0, const int x_stride, const int y_stride, const int z_stride)
{
